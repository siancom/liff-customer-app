...
if (content.includes(thaimartBanner)) {
    ...
    // Insert into shop
    const shopAnchor = `{activeNav === 'shop' && (
             <div className="space-y-6 animate-in fade-in duration-300">`;
    content = content.replace(shopAnchor, shopAnchor + thaimartBanner);
    
    fs.writeFileSync(appPath, content);
    console.log("Moved Thaimart banner to shop page.");
} else {
    console.log("Could not find Thaimart banner.");
}
